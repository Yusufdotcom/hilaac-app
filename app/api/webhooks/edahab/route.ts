 ...
 * Public webhook invoked by eDahab after a mobile money payment settles.
...
export async function POST(req: NextRequest) {
  ...
  const { error } = await supabase
    .from("orders")
    .update({ payment_status: paymentStatus, payment_method: "edahab", payment_reference: String(body.transactionId ?? reference) })
    .eq("id", order.id);
