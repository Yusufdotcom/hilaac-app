...
/**
 * PATCH /api/admin/events/bookings
 * Update status / payment fields. Confirmed bookings cannot double-book a space+date.
 */
export async function PATCH(req: NextRequest) {
  ...
  if ("deposit_paid" in body) {
    update.deposit_paid = Number(body.deposit_paid) || 0;
  }
  if ("balance_due_date" in body) {
    update.balance_due_date = body.balance_due_date ? String(body.balance_due_date) : null;
  }
  ...
  const spaceId = existing.space_id as string | null;
  const eventDate = existing.event_date as string;
